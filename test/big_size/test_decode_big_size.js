...
tests.forEach(({args, description, error, expected}) => {
  return test(description, ({end, equal, throws}) => {
    if (!!error) {
      ...
      const {decoded} = decodeBigSize(args);

      equal(decoded, expected.decoded, 'Got expected result');
    }
